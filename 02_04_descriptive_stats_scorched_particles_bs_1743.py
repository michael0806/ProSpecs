...
# MAGIC %md
# MAGIC ## This notebook is used for descriptive statistics of Scorched Particles (BS 1743).
...
# MAGIC %md
# MAGIC Convert the Scorched Particles (BS 1743) actual result to binary. Set values of 'A' to 1 (in spec) and other values to 0 (out of spec
...
filtered_df['TEST_ACTUAL_RESULT'] = filtered_df['TEST_ACTUAL_RESULT'].apply(lambda x: 1 if x == 'A' else 0)
...
plt.title("Count of Each TEST_ACTUAL_RESULT value for Scorched Particles (BS 1743)")
...
# MAGIC
# MAGIC Then check the relationship betwweent those varibales and test actual reslut for Scorched Particles (BS 1743).
...
plot_categorical_summary(filtered_df, 'PARAMETERS_VERSION_NUMBER', 'TEST_ACTUAL_RESULT', legend_loc='lower left')

# COMMAND ----------

# RUN_BATCH_NUMBER VS TEST_ACTUAL_RESULT
plot_categorical_summary(filtered_df, 'RUN_BATCH_NUMBER', 'TEST_ACTUAL_RESULT', legend_loc='lower left')

# COMMAND ----------

# BATCH_MILK_SOURCE VS TEST_ACTUAL_RESULT
plot_categorical_summary(filtered_df, 'BATCH_MILK_SOURCE', 'TEST_ACTUAL_RESULT', legend_loc='lower left')
...
# MAGIC
# MAGIC Then analyze the relationship between each processing parameter and the target variable which is the Scorched Particles (BS 1743) actual test reslut. 

# COMMAND ----------

# List the variables of interest for the evaporator variables
evap_vars = [
  "EVAPORATOR_NUMBER_IN_USE",
]

# Calculate the unique values of each variable
...
for var in appearance_vars:
    plot_categorical_summary(filtered_df, var, 'TEST_ACTUAL_RESULT', legend_loc='upper right')