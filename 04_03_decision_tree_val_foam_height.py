...
# MAGIC %md
# MAGIC ## This notebook is used for validating the best decision tree modelling for PPP Foam Height (DV).
...
X_foam_height_val = datasets["X_foam_height_val"]
y_foam_height_val = datasets["y_foam_height_val"]
...
model_path = "/Workspace/Repos/oce/michael.sun.onesource.APAC-OCE-db/src/ds_projects/prospecs/models/best_decision_tree_model_foam_height.pkl"
best_model = joblib.load(model_path)

# COMMAND ----------

# Predict using the best model on the validation set
y_val_pred = best_model.predict(X_foam_height_val)

# Evaluate performance on the validation set
print("Validation Accuracy:", accuracy_score(y_foam_height_val, y_val_pred))
print("Confusion Matrix:\n", confusion_matrix(y_foam_height_val, y_val_pred))
print("Classification Report:\n", classification_report(y_foam_height_val, y_val_pred))

# COMMAND ----------

# Calculate the ROC curve for the best model
y_val_prob_dt = best_model.predict_proba(X_foam_height_val)[:, 1]
fpr_dt, tpr_dt, _ = roc_curve(y_foam_height_val, y_val_prob_dt)
