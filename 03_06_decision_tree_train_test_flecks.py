...
# MAGIC %md
# MAGIC ## This notebook is used for decision tree modelling for PPP Flecks.
...
X_flecks_train = datasets["X_flecks_train_res"]
X_flecks_test = datasets["X_flecks_test_res"]
y_flecks_train = datasets["y_flecks_train_res"]
y_flecks_test = datasets["y_flecks_test_res"]
...
feature_names = X_flecks_train.columns
...
def evaluate_model(dt_classifier):
    ...
    print("Train Accuracy : {0:.3f}".format(
        accuracy_score(y_flecks_train, dt_classifier.predict(X_flecks_train))))
    print("Train Confusion Matrix:")
    print(confusion_matrix(y_flecks_train, dt_classifier.predict(X_flecks_train)))
    print(classification_report(y_flecks_train, dt_classifier.predict(X_flecks_train)))

    false_positive_rate, true_positive_rate, thresholds = roc_curve(
        y_flecks_train, dt_classifier.predict(X_flecks_train))
    ...
    print("Test Accuracy : {0:.3f}".format(
        accuracy_score(y_flecks_test, dt_classifier.predict(X_flecks_test))))
    print("Test Confusion Matrix:")
    print(confusion_matrix(y_flecks_test, dt_classifier.predict(X_flecks_test)))
    print(classification_report(y_flecks_test, dt_classifier.predict(X_flecks_test)))

    false_positive_rate_test, true_positive_rate_test, thresholds_test = roc_curve(
        y_flecks_test, dt_classifier.predict(X_flecks_test))
...
DT_default.fit(X_flecks_train, y_flecks_train)

# Predict the labels for the test set
y_pred = DT_default.predict(X_flecks_test)

# Calculate the accuracy of the model on the test set
DT_default_result = accuracy_score(y_flecks_test, y_pred)
...
# Fit the decision tree classifier on the training data
DT.fit(X_flecks_train, y_flecks_train)
...
dt_grid.fit(X_flecks_train, y_flecks_train)

# Output the best parameters
print("Best parameters for Decision Tree:", dt_grid.best_params_)

# Predict using the best model
y_test_pred_dt = dt_grid.predict(X_flecks_test)
...
_ = tree.plot_tree(
    ...
    feature_names=X_flecks_train.columns.tolist(),
    class_names=['0', '1'],
    fontsize=6,
    filled=True
)
