...
# MAGIC %md
# MAGIC ## This notebook is used for extracting and preparing the dataset for modeling PPP 'Bulk Density'.
...
# MAGIC %md
# MAGIC ### Step 2: Filter the data to focus on a specific subset of interest, in this case, rows where TEST_FULL_CODE is ‘Bulk Density’.
...
# Define the SQL query to select rows with SKU code 10449526 and TEST_FULL_CODE = 'Bulk Density'
query = """
SELECT * 
FROM snowflake_table
WHERE SKU_NUMBER = '10449526' AND TEST_FULL_CODE IN ('Bulk Density')
"""
...
# MAGIC %md
# MAGIC Convert the bulk density actual result to binary. Set values between 0.49 and 0.51 (inclusive) to 1, meaning in_spec, and set the rest values to 0, meaning oo_spec, for future data modeling.
...
# Convert the 'TEST_ACTUAL_RESULT' column to floats
new_df['TEST_ACTUAL_RESULT'] = new_df['TEST_ACTUAL_RESULT'].astype(float)

# Set values between 0.49 and 0.51 (inclusive) as 1, and the rest as 0
new_df['TEST_ACTUAL_RESULT'] = new_df['TEST_ACTUAL_RESULT'].apply(lambda x: 1 if 0.49 <= x <= 0.51 else 0)
...
X_bd_train, X_bd_temp, y_bd_train, y_bd_temp = train_test_split(
    X, y, test_size=0.3, random_state=42
)

# Split temporary set into test and validation sets
X_bd_test, X_bd_val, y_bd_test, y_bd_val = train_test_split(
    X_bd_temp, y_bd_temp, test_size=0.5, random_state=42
)  # Split temp into 50% test, 50% validation

# Apply SMOTE only to the training and test sets
oversample = SMOTE()

X_bd_train = X_bd_train.apply(pd.to_numeric, errors='coerce')
y_bd_train = pd.to_numeric(y_bd_train, errors='coerce')

X_bd_test = X_bd_test.apply(pd.to_numeric, errors='coerce')
y_bd_test = pd.to_numeric(y_bd_test, errors='coerce')

# Print counts before resampling
print("Before OverSampling, counts of label '1' in y train: {}".format(sum(y_bd_train == 1)))
print("Before OverSampling, counts of label '0' in y train: {} \n".format(sum(y_bd_train == 0)))

print("Before OverSampling, counts of label '1' in y test: {}".format(sum(y_bd_test == 1)))
print("Before OverSampling, counts of label '0' in y test: {} \n".format(sum(y_bd_test == 0)))

X_bd_train_res, y_bd_train_res = oversample.fit_resample(X_bd_train, y_bd_train)
X_bd_test_res, y_bd_test_res = oversample.fit_resample(X_bd_test, y_bd_test)

print('After OverSampling, the shape of X_train_res: {}'.format(X_bd_train_res.shape))
print('After OverSampling, the shape of y_train_res: {} \n'.format(y_bd_train_res.shape))

print('After OverSampling, the shape of X_test_res: {}'.format(X_bd_test_res.shape))
print('After OverSampling, the shape of y_test_res: {} \n'.format(y_bd_test_res.shape))

print("After OverSampling, counts of label '1' in y train_res: {}".format(sum(y_bd_train_res == 1)))
print("After OverSampling, counts of label '0' in y train_res: {}".format(sum(y_bd_train_res == 0)))

print("After OverSampling, counts of label '1' in y test_res: {}".format(sum(y_bd_test_res == 1)))
print("After OverSampling, counts of label '0' in y test_res: {}".format(sum(y_bd_test_res == 0)))

# Check the count of the validation dataset
print("Validation dataset shape: {}".format(X_bd_val.shape))
print("Counts of label '1' in validation: {}".format(sum(y_bd_val == 1)))
print("Counts of label '0' in validation: {}".format(sum(y_bd_val == 0)))

# COMMAND ----------

# Check and convert y_bd, y_bd_val to pandas DataFrame 
datasets_to_check = {
    "y_bd_train_res": y_bd_train_res,
    "y_bd_test_res": y_bd_test_res,
    "y_bd_val": y_bd_val,
}
...
y_bd_train_res = datasets_to_check["y_bd_train_res"]
y_bd_test_res = datasets_to_check["y_bd_test_res"]
y_bd_val = datasets_to_check["y_bd_val"]
...
datasets = {
    "X_bd_train_res": X_bd_train_res,
    "X_bd_test_res": X_bd_test_res,
    "X_bd_val": X_bd_val,
