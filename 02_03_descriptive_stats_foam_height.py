...
# MAGIC %md
# MAGIC ## This notebook is used for descriptive statistics of PPP Foam Height (DV).
...
# MAGIC %md
# MAGIC Convert the form height actual result to binary. Set values of 10 and 20 to 1 (in spec) and the rest of the values (30, 40, 50) to 0 (out of spec).
...
filtered_df['TEST_ACTUAL_RESULT'] = filtered_df['TEST_ACTUAL_RESULT'].apply(lambda x: 1 if x in [10, 20] else 0)
...
plt.title("Count of Each TEST_ACTUAL_RESULT value for Foam Height")
...
# MAGIC
# MAGIC Then check the relationship betwweent those varibales and test actual result for Foam Height (DV).
...
# Check the unique values for the batch variables
...
plot_categorical_summary(filtered_df, 'RUN_BATCH_NUMBER', 'TEST_ACTUAL_RESULT', legend_loc='upper left')
...
# MAGIC
# MAGIC Then analyze the relationship between each processing parameter and the target variable which is the Foam Height (DV) actual test result. 
...
# Check the unique values for the evaporator variables
...
for var in external_vars:
    plot_categorical_summary(filtered_df, var, 'TEST_ACTUAL_RESULT',  legend_loc='upper center')
...
# Calculate statistics for the new variables
