...
# MAGIC %md
# MAGIC ## This notebook is used for descriptive statistics of Bulk Density.
...
# MAGIC %md
# MAGIC Convert the bulk density actual result to binary. Set values between 0.49 and 0.51 (inclusive) to 1, meaning in spec, and set values less than 0.49 or greater than 0.51 to 0, meaning out of spec.
...
filtered_df['TEST_ACTUAL_RESULT'] = filtered_df['TEST_ACTUAL_RESULT'].astype(float)

# Set values between 0.49 and 0.51 (inclusive) as 1, and the rest as 0
filtered_df['TEST_ACTUAL_RESULT'] = filtered_df['TEST_ACTUAL_RESULT'].apply(lambda x: 1 if 0.49 <= x <= 0.51 else 0)
...
plt.title("Count of Each TEST_ACTUAL_RESULT value for Bulk Density")
...
# MAGIC
# MAGIC Then check the relationship betwweent those varibales and test actual result for Bulk Density.
...
# MAGIC
# MAGIC Then analyze the relationship between each processing parameter and the target variable which is the Bulk Density actual test result. 
...
# List the variables of interest for density part
density_vars = [
  "PROCESS_BULK_DENSITY",
  "PACKING_BULK_DENSITY"
]
