...
# MAGIC %md
# MAGIC ## This notebook is used for decision tree modelling for PPP Foam Height (DV).
...
datasets_to_read = [
    "X_foam_height_train_res",
    "X_foam_height_test_res",
    "y_foam_height_train_res",
    "y_foam_height_test_res"
]
...
X_foam_height_train = datasets["X_foam_height_train_res"]
X_foam_height_test = datasets["X_foam_height_test_res"]
y_foam_height_train = datasets["y_foam_height_train_res"]
y_foam_height_test = datasets["y_foam_height_test_res"]
...
feature_names = X_foam_height_train.columns
...
def evaluate_model(dt_classifier):
    ...
    print("Train Accuracy : {0:.3f}".format(
        accuracy_score(y_foam_height_train, dt_classifier.predict(X_foam_height_train))))
    print("Train Confusion Matrix:")
    print(confusion_matrix(y_foam_height_train, dt_classifier.predict(X_foam_height_train)))
    print(classification_report(y_foam_height_train, dt_classifier.predict(X_foam_height_train)))

    false_positive_rate, true_positive_rate, thresholds = roc_curve(
        y_foam_height_train, dt_classifier.predict(X_foam_height_train))
    ...
    print("Test Accuracy : {0:.3f}".format(
        accuracy_score(y_foam_height_test, dt_classifier.predict(X_foam_height_test))))
    print("Test Confusion Matrix:")
    print(confusion_matrix(y_foam_height_test, dt_classifier.predict(X_foam_height_test)))
    print(classification_report(y_foam_height_test, dt_classifier.predict(X_foam_height_test)))

    false_positive_rate_test, true_positive_rate_test, thresholds_test = roc_curve(
        y_foam_height_test, dt_classifier.predict(X_foam_height_test))
...
DT_default.fit(X_foam_height_train, y_foam_height_train)

# Predict the labels for the test set
y_pred = DT_default.predict(X_foam_height_test)

# Calculate the accuracy of the model on the test set
DT_default_result = accuracy_score(y_foam_height_test, y_pred)
...
# MAGIC %md
# MAGIC ### Set max_depth = 15

# COMMAND ----------

# Define the DecisionTreeClassifier with a maximum depth of 15 and a fixed random state
DT_default = DecisionTreeClassifier(max_depth=15, random_state= RANDOM_STATE)
...
# Define the classifier with a maximum depth of 15 and a fixed random state
DT = DecisionTreeClassifier(max_depth=15, random_state= RANDOM_STATE)

# Fit the decision tree classifier on the training data
DT.fit(X_foam_height_train, y_foam_height_train)
...
# MAGIC
# MAGIC  In summary, the cross-validation results indicate that the Decision Tree classifier has an average accuracy of 54.0% on the training data, with a variability of ±1.6% across the different folds.
...
# MAGIC %md
# MAGIC The model performs better on the training set compared to the test set, indicating potential overfitting.
# MAGIC The precision, recall, and F1-score for class 1 (positive class) are higher than for class 0 (negative class) in both training and test sets.
# MAGIC The model has a higher recall for class 1, meaning it is better at identifying positive instances but has a lower precision for class 0, indicating more false positives.
...
dt_grid.fit(X_foam_height_train, y_foam_height_train)

# Output the best parameters
print("Best parameters for Decision Tree:", dt_grid.best_params_)

# Predict using the best model
y_test_pred_dt = dt_grid.predict(X_foam_height_test)
...
# MAGIC %md
# MAGIC Overall, the model shows good performance on both the training and test sets, with slightly better performance on the test set, indicating that it generalizes well to unseen data.
# MAGIC
...
_ = tree.plot_tree(
    ...
    feature_names=X_foam_height_train.columns.tolist(),
    class_names=['0', '1'],
    fontsize=6,
    filled=True
)
...
model_filename = "/Workspace/Repos/oce/michael.sun.onesource.APAC-OCE-db/src/ds_projects/prospecs/models/best_decision_tree_model_foam_height.pkl"
