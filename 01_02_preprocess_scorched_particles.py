...
# MAGIC %md
# MAGIC ## This notebook is used for extracting and preparing the dataset for modeling PPP Scorched Particles (DV).
...
# MAGIC %md
# MAGIC ### Step 2: Filter the data to focus on a specific subset of interest, in this case, rows where TEST_FULL_CODE is ‘Scorched Particles (DV)’.
...
WHERE SKU_NUMBER = '10449526' AND TEST_FULL_CODE IN ('Scorched Particles (DV)')
"""
...
# MAGIC %md
# MAGIC The above columns that were removed were confirmed with D&I Vincent, as those variables are not useful for the project. "EFB_SECTION_1_AIR_PRESSURE", "EFB_SECTION_2_AIR_PRESSURE", "EFB_SECTION_3_AIR_PRESSURE", "EFB_1_AIR_TEMPERATURE_TARGET", "EFB_2_AIR_TEMPERATURE_TARGET", "EFB_3_AIR_TEMPERATURE_TARGET" variables removed due to data confudion.

# COMMAND ----------

# Check updated filtered dataframe
filtered_df.info()
...
# MAGIC %md
# MAGIC Convert the scorched particles actual result to binary. Set 0, 1, 2, and 3 to 1, meaning in_spec, and set other values to 0, meaning oo_spec, for future data modeling.
...
# Filter out rows where 'TEST_ACTUAL_RESULT' is '#'
new_df = new_df[new_df['TEST_ACTUAL_RESULT'] != '#']

# Check the value count in 'TEST_ACTUAL_RESULT'
...
new_df.loc[:, 'TEST_ACTUAL_RESULT'] = new_df['TEST_ACTUAL_RESULT'].astype(int)

# Set 0, 1, 2, and 3 as 1, and other values as 0 in the 'TEST_ACTUAL_RESULT' column
new_df.loc[:, 'TEST_ACTUAL_RESULT'] = new_df['TEST_ACTUAL_RESULT'].apply(lambda x: 1 if x in [0, 1, 2, 3] else 0)
...
# Identify categorical variables in new_df
...
for col1 in categorical_columns:
    for col2 in categorical_columns:
        if col1 != col2:
            contingency_table = pd.crosstab(new_df[col1], new_df[col2])
            chi2, p, dof, ex = chi2_contingency(contingency_table)
            chi2_results[(col1, col2)] = p

# Identify highly correlated variables (p-value < 0.05)
highly_correlated = [pair for pair, p_value in chi2_results.items() if p_value < 0.05]
...
print("Removed columns due to high correlation:", removed_columns)

# Drop the removed columns from new_df
new_df = new_df.drop(columns=list(removed_columns))

# COMMAND ----------

display(new_df['TEST_ACTUAL_RESULT'].value_counts())
new_df.info()
...
X_scorched_particles_train, X_scorched_particles_temp, y_scorched_particles_train, y_scorched_particles_temp = train_test_split(
    X, y, test_size=0.3, random_state=42
)

# Split temporary set into test and validation sets
X_scorched_particles_test, X_scorched_particles_val, y_scorched_particles_test, y_scorched_particles_val = train_test_split(
    X_scorched_particles_temp, y_scorched_particles_temp, test_size=0.5, random_state=42
)  # Split temp into 50% test, 50% validation

# Apply SMOTE only to the training and test sets
oversample = SMOTE()

X_scorched_particles_train = X_scorched_particles_train.apply(pd.to_numeric, errors='coerce')
y_scorched_particles_train = pd.to_numeric(y_scorched_particles_train, errors='coerce')

X_scorched_particles_test = X_scorched_particles_test.apply(pd.to_numeric, errors='coerce')
y_scorched_particles_test = pd.to_numeric(y_scorched_particles_test, errors='coerce')

# Print counts before resampling
print("Before OverSampling, counts of label '1' in y train: {}".format(sum(y_scorched_particles_train == 1)))
print("Before OverSampling, counts of label '0' in y train: {} \n".format(sum(y_scorched_particles_train == 0)))

print("Before OverSampling, counts of label '1' in y test: {}".format(sum(y_scorched_particles_test == 1)))
print("Before OverSampling, counts of label '0' in y test: {} \n".format(sum(y_scorched_particles_test == 0)))

X_scorched_particles_train_res, y_scorched_particles_train_res = oversample.fit_resample(X_scorched_particles_train, y_scorched_particles_train)
X_scorched_particles_test_res, y_scorched_particles_test_res = oversample.fit_resample(X_scorched_particles_test, y_scorched_particles_test)

print('After OverSampling, the shape of X_train_res: {}'.format(X_scorched_particles_train_res.shape))
print('After OverSampling, the shape of y_train_res: {} \n'.format(y_scorched_particles_train_res.shape))

print('After OverSampling, the shape of X_test_res: {}'.format(X_scorched_particles_test_res.shape))
print('After OverSampling, the shape of y_test_res: {} \n'.format(y_scorched_particles_test_res.shape))

print("After OverSampling, counts of label '1' in y train_res: {}".format(sum(y_scorched_particles_train_res == 1)))
print("After OverSampling, counts of label '0' in y train_res: {}".format(sum(y_scorched_particles_train_res == 0)))

print("After OverSampling, counts of label '1' in y test_res: {}".format(sum(y_scorched_particles_test_res == 1)))
print("After OverSampling, counts of label '0' in y test_res: {}".format(sum(y_scorched_particles_test_res == 0)))

# Check the count of the validation dataset
print("Validation dataset shape: {}".format(X_scorched_particles_val.shape))
print("Counts of label '1' in validation: {}".format(sum(y_scorched_particles_val == 1)))
print("Counts of label '0' in validation: {}".format(sum(y_scorched_particles_val == 0)))

# COMMAND ----------

# Check and convert y_scorched_particles, y_scorched_particles_val to pandas DataFrame 
datasets_to_check = {
    "y_scorched_particles_train_res": y_scorched_particles_train_res,
    "y_scorched_particles_test_res": y_scorched_particles_test_res,
    "y_scorched_particles_val": y_scorched_particles_val,
}
...
y_scorched_particles_train_res = datasets_to_check["y_scorched_particles_train_res"]
y_scorched_particles_test_res = datasets_to_check["y_scorched_particles_test_res"]
y_scorched_particles_val = datasets_to_check["y_scorched_particles_val"]
...
datasets = {
    "X_scorched_particles_train_res": X_scorched_particles_train_res,
    "X_scorched_particles_test_res": X_scorched_particles_test_res,
    "X_scorched_particles_val": X_scorched_particles_val,
