...
# MAGIC %md
# MAGIC ## This notebook is used for descriptive statistics of PPP Scorched Particles (DV).
...
# MAGIC %md
# MAGIC Convert the scorched particles actual result to binary. Set values of 0, 1, 2, and 3 to 1 (in spec) and other values to 0 (out of spec).
...
filtered_df = filtered_df[filtered_df['TEST_ACTUAL_RESULT'] != '#']
...
filtered_df.loc[:, 'TEST_ACTUAL_RESULT'] = filtered_df['TEST_ACTUAL_RESULT'].astype(int)

# Set 0, 1, 2, and 3 as 1, and other values as 0 in the 'TEST_ACTUAL_RESULT' column
filtered_df.loc[:, 'TEST_ACTUAL_RESULT'] = filtered_df['TEST_ACTUAL_RESULT'].apply(lambda x: 1 if x in [0, 1, 2, 3] else 0)
...
plt.title("Count of Each TEST_ACTUAL_RESULT value for Scorched Particles")
...
# MAGIC
# MAGIC Then check the relationship betwweent those varibales and test actual result for Scorched Particles (DV).
...
# Show the unique values for the batch variables
...
# MAGIC
# MAGIC Then analyze the relationship between each processing parameter and the target variable which is the Scorched Particles (DV) actual test result. 
...
# Check for unique values for EVAPORATOR_NUMBER_IN_USE
...
# EVAPORATOR_NUMBER_IN_USE VS TEST_ACTUAL_RESULT
...
# Calculate statistics for the evaporator stage variables
...
# Create a faceted boxplot for the evaporator stage variables and TEST_ACTUAL_RESULT
...
# Display the statistics table for the evaporator stage variables and TEST_ACTUAL_RESULT
display(stats)
...
sns.boxplot(data=filtered_df, x='TEST_ACTUAL_RESULT', y='SECOND_STAGE_DSI_TEMPERATURE')
plt.title('SECOND_STAGE_DSI_TEMPERATURE vs TEST_ACTUAL_RESULT')
plt.xticks(rotation=45)
plt.show()
...
# Calculate statistics for oil addition & concentrate preparation variables
...
# Display the statistics table for the oil addition & concentrate preparation variables and TEST_ACTUAL_RESULT
display(stats)
...
# Calculate the unique values for each variable
...
# List the density variables
density_vars = [
  "PROCESS_BULK_DENSITY",
  "PACKING_BULK_DENSITY"
]

# Calculate statistics for the density variables
