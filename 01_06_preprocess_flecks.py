...
# MAGIC %md
# MAGIC ## This notebook is used for extracting and preparing the dataset for modeling PPP Flecks.
...
# MAGIC %md
# MAGIC ### Step 2: Filter the data to focus on a specific subset of interest, in this case, rows where TEST_FULL_CODE is ‘Flecks’.
...
# Define the SQL query to select rows with SKU code 10449526 and TEST_FULL_CODE = 'Flecks'
query = """
SELECT * 
...
# MAGIC %md
# MAGIC  Remove the rows with null values, because we still interested in the relationship between those variables and target value.
...
# MAGIC %md
# MAGIC Convert the flecks actual result to binary. Set A+ and A- to 1, meaning in_spec, and set remaining results to 0, meaning oo_spec, for future data modeling.
...
new_df['TEST_ACTUAL_RESULT'] = new_df['TEST_ACTUAL_RESULT'].apply(lambda x: 1 if x in ['A+', 'A-'] else 0)
...
X_flecks_train, X_flecks_temp, y_flecks_train, y_flecks_temp = train_test_split(
    X, y, test_size=0.3, random_state=42
)

# Split temporary set into test and validation sets
X_flecks_test, X_flecks_val, y_flecks_test, y_flecks_val = train_test_split(
    X_flecks_temp, y_flecks_temp, test_size=0.5, random_state=42
)  # Split temp into 50% test, 50% validation

# Apply SMOTE only to the training and test sets
oversample = SMOTE()

X_flecks_train = X_flecks_train.apply(pd.to_numeric, errors='coerce')
y_flecks_train = pd.to_numeric(y_flecks_train, errors='coerce')

X_flecks_test = X_flecks_test.apply(pd.to_numeric, errors='coerce')
y_flecks_test = pd.to_numeric(y_flecks_test, errors='coerce')

# Print counts before resampling
print("Before OverSampling, counts of label '1' in y train: {}".format(sum(y_flecks_train == 1)))
print("Before OverSampling, counts of label '0' in y train: {} \n".format(sum(y_flecks_train == 0)))

print("Before OverSampling, counts of label '1' in y test: {}".format(sum(y_flecks_test == 1)))
print("Before OverSampling, counts of label '0' in y test: {} \n".format(sum(y_flecks_test == 0)))

X_flecks_train_res, y_flecks_train_res = oversample.fit_resample(X_flecks_train, y_flecks_train)
X_flecks_test_res, y_flecks_test_res = oversample.fit_resample(X_flecks_test, y_flecks_test)

print('After OverSampling, the shape of X_train_res: {}'.format(X_flecks_train_res.shape))
print('After OverSampling, the shape of y_train_res: {} \n'.format(y_flecks_train_res.shape))

print('After OverSampling, the shape of X_test_res: {}'.format(X_flecks_test_res.shape))
print('After OverSampling, the shape of y_test_res: {} \n'.format(y_flecks_test_res.shape))

print("After OverSampling, counts of label '1' in y train_res: {}".format(sum(y_flecks_train_res == 1)))
print("After OverSampling, counts of label '0' in y train_res: {}".format(sum(y_flecks_train_res == 0)))

print("After OverSampling, counts of label '1' in y test_res: {}".format(sum(y_flecks_test_res == 1)))
print("After OverSampling, counts of label '0' in y test_res: {}".format(sum(y_flecks_test_res == 0)))

# Check the count of the validation dataset
print("Validation dataset shape: {}".format(X_flecks_val.shape))
print("Counts of label '1' in validation: {}".format(sum(y_flecks_val == 1)))
print("Counts of label '0' in validation: {}".format(sum(y_flecks_val == 0)))

# COMMAND ----------

# Check and convert y_flecks, y_flecks_val to pandas DataFrame 
datasets_to_check = {
    "y_flecks_train_res": y_flecks_train_res,
    "y_flecks_test_res": y_flecks_test_res,
    "y_flecks_val": y_flecks_val,
}
...
y_flecks_train_res = datasets_to_check["y_flecks_train_res"]
y_flecks_test_res = datasets_to_check["y_flecks_test_res"]
y_flecks_val = datasets_to_check["y_flecks_val"]
...
datasets = {
    "X_flecks_train_res": X_flecks_train_res,
    "X_flecks_test_res": X_flecks_test_res,
    "X_flecks_val": X_flecks_val,
