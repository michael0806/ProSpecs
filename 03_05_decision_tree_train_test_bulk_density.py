...
# MAGIC %md
# MAGIC ## This notebook is used for decision tree modelling for PPP Bulk Density.
...
X_bd_train = datasets["X_bd_train_res"]
X_bd_test = datasets["X_bd_test_res"]
y_bd_train = datasets["y_bd_train_res"]
y_bd_test = datasets["y_bd_test_res"]
...
feature_names = X_bd_train.columns
...
def evaluate_model(dt_classifier):
    ...
    print("Train Accuracy : {0:.3f}".format(
        accuracy_score(y_bd_train, dt_classifier.predict(X_bd_train))))
    print("Train Confusion Matrix:")
    print(confusion_matrix(y_bd_train, dt_classifier.predict(X_bd_train)))
    print(classification_report(y_bd_train, dt_classifier.predict(X_bd_train)))

    false_positive_rate, true_positive_rate, thresholds = roc_curve(
        y_bd_train, dt_classifier.predict(X_bd_train))
    ...
    print("Test Accuracy : {0:.3f}".format(
        accuracy_score(y_bd_test, dt_classifier.predict(X_bd_test))))
    print("Test Confusion Matrix:")
    print(confusion_matrix(y_bd_test, dt_classifier.predict(X_bd_test)))
    print(classification_report(y_bd_test, dt_classifier.predict(X_bd_test)))

    false_positive_rate_test, true_positive_rate_test, thresholds_test = roc_curve(
        y_bd_test, dt_classifier.predict(X_bd_test))
...
DT_default.fit(X_bd_train, y_bd_train)

# Predict the labels for the test set
y_pred = DT_default.predict(X_bd_test)

# Calculate the accuracy of the model on the test set
DT_default_result = accuracy_score(y_bd_test, y_pred)
...
# MAGIC %md
# MAGIC ### Set max_depth = 25

# COMMAND ----------

# Define the DecisionTreeClassifier with a maximum depth of  and a fixed random state
DT_default = DecisionTreeClassifier(max_depth=25, random_state= RANDOM_STATE)
...
DT = DecisionTreeClassifier(max_depth=25, random_state= RANDOM_STATE)

# Fit the decision tree classifier on the training data
DT.fit(X_bd_train, y_bd_train)
...
dt_grid.fit(X_bd_train, y_bd_train)

# Output the best parameters
print("Best parameters for Decision Tree:", dt_grid.best_params_)

# Predict using the best model
y_test_pred_dt = dt_grid.predict(X_bd_test)
...
_ = tree.plot_tree(
    ...
    feature_names=X_bd_train.columns.tolist(),
    class_names=['0', '1'],
    fontsize=6,
    filled=True
)
...
model_filename = "/Workspace/Repos/oce/michael.sun.onesource.APAC-OCE-db/src/ds_projects/prospecs/models/best_decision_tree_model_bulk_density.pkl"
