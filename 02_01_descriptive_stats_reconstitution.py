...
# MAGIC %md
# MAGIC ## This notebook is used for descriptive statistics of PPP Reconstitution (DV).
...
WHERE SKU_NUMBER = '10449526' AND TEST_FULL_CODE IN ('Reconstitution (DV)')
"""
...
# MAGIC %md
# MAGIC Convert the reconstitution actual result to binary. Set values of 0 and 1 to 1 (in spec) and values of 2 and 3 to 0 (out of spec).
...
filtered_df['TEST_ACTUAL_RESULT'] = filtered_df['TEST_ACTUAL_RESULT'].astype(int)

# Set 0 and 1 as 1, and 2 and 3 as 0 in the 'TEST_ACTUAL_RESULT' column
filtered_df['TEST_ACTUAL_RESULT'] = filtered_df['TEST_ACTUAL_RESULT'].map({0: 1, 1: 1, 2: 0, 3: 0})
...
plt.title("Count of Each TEST_ACTUAL_RESULT value for Recon")
...
# MAGIC
# MAGIC Then check the relationship betwweent those varibales and test actual result for reconstitution.
...
bar_plot = sns.barplot(data=result_summary, x='PO_NUMBER', y='Count', hue='TEST_ACTUAL_RESULT')
...
# MAGIC
# MAGIC Then analyze the relationship between each processing parameter and the target variable which is the reconstitution actual test result. 

# COMMAND ----------

# List the variables of interest of the evaporator stage
evap_vars = [
...
# MAGIC %md
# MAGIC Define functions for future use.
...
g.set_titles("{col_name}")
g.set_axis_labels("TEST_ACTUAL_RESULT", "Value")
g.fig.suptitle("OIL ADDITION & CONCENTRATE PREPARATION Variables vs. TEST_ACTUAL_RESULT Value", fontsize=14, fontweight='bold')
g.fig.subplots_adjust(top=0.9)
plt.xticks(rotation=45)
plt.show()
...
# Calculate statistics for the drying stage part 1 variables
...
# Calculate statistics for the stage drying part 2 variables
